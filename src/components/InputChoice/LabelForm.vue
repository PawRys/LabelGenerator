...
<template>
	<form class="" @submit.prevent="addItem">
		<input
			...
			name="itemSize"
			value="12x1250x2500"
			class="form__item-size"
			placeholder="format"
			pattern="\d+([,.]\d)?x\d+x\d+"
			required
			autocomplete="on" />
		<textarea type="text" name="longDesc" class="form__long-desc" placeholder="klasa" autocomplete="on">BB/CP</textarea>
		<section class="short-fields">
			<select name="itemGlue" value="MR" class="form__item-glue short-input" required>
		...
		<input type="text" value="contract no" name="contract" class="contractNo" placeholder="klient" autocomplete="on" />
